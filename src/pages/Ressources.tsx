...
import { Calendar, BookOpen, Headphones, Trophy, Video, Clock, Users } from "lucide-react";
...
// --- Icônes pour types d'articles ---
const getTypeIcon = (type: string) => {
  switch (type) {
    case "Article": return <BookOpen className="w-4 h-4" />;
    case "Tutoriel": return <Video className="w-4 h-4" />;
    case "Podcast": return <Headphones className="w-4 h-4" />;
    default: return <BookOpen className="w-4 h-4" />;
  }
};

// --- Icônes pour types d'événements ---
const getEventIcon = (type: string) => {
  switch (type) {
    case "Webinaire": return <Video className="w-5 h-5" />;
...
const ResourceCard = ({ article }: { article: typeof articles[0] }) => (
    ...
    <div className="relative h-48 w-full overflow-hidden">
      ...
      <div className="absolute top-4 left-4">
        <Badge className="bg-background/90 text-foreground flex items-center gap-1">
          {getTypeIcon(article.type)} <span>{article.type}</span>
        </Badge>
      </div>
    </div>
    <CardHeader>
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-sm text-muted-foreground flex items-center gap-1">
          <Clock className="w-4 h-4" /> {article.duree}
        </span>
      </div>
...
// --- Composant EventCard ---
const EventCard = ({ event }: { event: typeof evenements[0] }) => (
